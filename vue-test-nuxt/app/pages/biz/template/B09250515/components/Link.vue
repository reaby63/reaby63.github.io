...
<template>
    <div v-if="partnershipList?.length" :class="$style['link-space']">
        <div :class="$style.title">
            {{ commonStore.getPageInfo.referralLinkSet?.title || '相關連結 /Links' }}
        </div>
...
.title {
    ...
    color: #EB1B26F0;
    padding-bottom: 21px;
    border-bottom: solid 1px rgba(235, 27, 38, 0.94);
    margin-bottom: 20px;
}
...
.switch-btn {
    ...
    .switch-list {
        ...
        span {
            ...
            mask-size: contain;
            background: rgba(235, 27, 38, 0.94);
        }
    }
    .switch-box {
        margin-right: 3px;
...
.swiper-arrow {
    ...
    overflow: hidden;
    cursor: pointer;
    background: url('/images/biz/B09250515/link-arrow.svg') no-repeat center center;
